package com.github.ca_dmin.fakegps_for_tesla_android.service;
...
import com.github.ca_dmin.fakegps_for_tesla_android.service.looper.LocationThreadManager;
import com.github.ca_dmin.fakegps_for_tesla_android.ui.MainActivity;
...
public class LocationService extends Service {
    ...
    private final static String EXTRA_USE_REST_API    = "USE_REST_API";
    ...
    private LocPoint processIntentExtras(Intent intent) {
        ...
        boolean use_rest_api   = intent.getBooleanExtra(EXTRA_USE_REST_API, false);

        if (use_rest_api) {
            LTM.startRestApiMode();
        } else {
            LTM.stopRestApiMode();
        }
    ...
    public static Intent doStart(Context context, boolean broadcast, LocPoint origin, LocPoint destination, int trip_duration, boolean is_rest_api) {
        if (origin == null)
            return null;

        Intent intent = new Intent(context, LocationService.class);
        addIntentExtras(intent, origin, destination, trip_duration, is_rest_api);
        return doAction(context, intent, ACTION_START, broadcast);
    }

    private static void addIntentExtras(Intent intent, LocPoint origin, LocPoint destination, int trip_duration, boolean is_rest_api) {
        ...
        if (is_rest_api) {
            intent.putExtra(EXTRA_USE_REST_API, is_rest_api);
        }
    }
